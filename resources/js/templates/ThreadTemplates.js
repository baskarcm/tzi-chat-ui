window.ThreadTemplates = (function () {
    ...
    templates = {
        ...
        thread_group_header : function(data){
            var knok = '',
            /*var knok = '<button onclick="ThreadManager.calls().sendKnock()" id="knok_btn" data-toggle="tooltip" title="Knock at ' + Messenger.format().escapeHtml(data.name) + '" ' + 'data-placement="bottom" class="btn btn-lg text-secondary btn-light pt-1 pb-0 px-2" type="button"><i class="fas fa-hand-rock fa-2x"></i></button>',*/
            ...
            //   <div class="dropdown float-right">\n' + templates.thread_socket_error(true) + '<span id="thread_option_call">' + templates.thread_call_state(data) + '</span>\n' + (!data.locked && data.options.knock ? knok : '') + '    <button class="btn btn-lg text-secondary btn-light dropdown-toggle pt-1 pb-0 px-2" type="button" data-toggle="dropdown"><i class="fas fa-cog fa-2x"></i></button>\n' + '    <div class="dropdown-menu dropdown-menu-right">\n' + '        <div class="dropdown-header py-0 h6 text-dark"><img id="group_avatar_' + data.id + '" alt="Group Image" class="show_group_avatar_' + data.id + ' rounded-circle small_img" src="' + data.avatar.sm + '"/>' + '           <span id="group_name_area">' + data.name + '</span></div>\n' + templates.thread_resource_dropdown() + view_participants + (!data.locked && data.options.chat_bots ? view_bots : '') + (!data.locked && data.options.invitations ? invites : '') + (!data.locked && data.options.admin ? admin : '') + '<div class="dropdown-divider"></div>' + (data.locked ? '' : data.options.muted ? '<a onclick="ThreadManager.mute().unmute(); return false;" class="dropdown-item" href="#"><i class="fas fa-volume-up"></i> Unmute</a>' : '<a onclick="ThreadManager.mute().mute(); return false;" class="dropdown-item" href="#"><i class="fas fa-volume-mute"></i> Mute</a>') + '<a class="dropdown-item" onclick="ThreadManager.group().leaveGroup(); return false;" id="leaveGroupLink" href="#"><i class="fas fa-sign-out-alt"></i> Leave Group</a>' + '</div>' + '<button onclick="ThreadManager.load().closeOpened()" title="Close" class="btn btn-lg text-danger btn-light pt-1 pb-0 px-2 mr-1" type="button"><i class="fas fa-times fa-2x"></i></button>' + '</div>
            return '<div id="thread_header_area">' + '<img onclick="ThreadTemplates.render().show_thread_avatar(\'' + data.avatar.lg + '\')" data-toggle="tooltip" data-placement="right" title="' + Messenger.format().escapeHtml(data.name) + '" class="ml-1 rounded-circle medium-image main-bobble-online pointer_area" src="' + data.avatar.sm + '" />' + '<div id="thread_error_area"></div></div>';
        },
